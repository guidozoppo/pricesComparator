...
export async function leerExcelNegocio(excelFile) {
    ...
    if (columnas.articulo === -1 || columnas.nombre === -1 || columnas.precioFinal === -1) {
        throw new Error("No se encontraron las columnas ARTICULO, NOMBRE y PRECIO FINAL.");
    }
...
function buscarColumnasNegocio(filas) {
    ...
    for (let numeroFila = 0; numeroFila < filas.length; numeroFila++) {
        ...
        if (columnaArticulo !== -1 && columnaNombre !== -1 && columnaPrecioFinal !== -1) {
            return {
